<template>
  <div class="details-box">
    <div class="vehicle-header">
      ...
      <div class="vehicle-info">
        ...
        <div class="vehicle-rating">
          ...
          <span
            >{{ overallRating.toFixed(1) }} ({{ totalReviews }} ratings)</span
          >
        </div>
      </div>
    </div>
    ...
    <div v-if="!isMotorcycle" class="details-list">
      <h4>Vehicle Features</h4>
      <div
        v-if="selectedFeaturesList.length > 0"
        class="features-grid-container"
      >
        <div class="features-grid">
          <ul class="feature-column">
            <li
              v-for="(feature, index) in selectedFeaturesList.slice(
                0,
                Math.ceil(selectedFeaturesList.length / 2)
              )"
              :key="`col1-${index}`"
            >
              {{ feature }}
    ...
    <div v-else class="details-list">
      <h4>Motorcycle Specifications</h4>
      <div class="specs-grid">
        <div class="spec-item">
          <span class="spec-label">Type:</span>
          <span class="spec-value">{{ vehicle.motorcycleType || "N/A" }}</span>
        </div>
        <div class="spec-item">
          <span class="spec-label">Engine:</span>
          <span class="spec-value"
            >{{ vehicle.engineDisplacement || "N/A" }} cc</span
          >
        </div>
        <div class="spec-item">
          <span class="spec-label">Year:</span>
          <span class="spec-value">{{ vehicle.year || "N/A" }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="rules-section">
      ...
      <p>
        <strong class="rule-title">No smoking allowed</strong> Smoking in any
        vehicle will result in a fine.
      </p>
      <p>
        <strong class="rule-title">Keep the vehicle tidy</strong> Unreasonably
        dirty vehicles may result in a fine.
      </p>
      <p>
        <strong class="rule-title">Refuel the vehicle</strong> Missing fuel may
        result in an additional fee.
      </p>
      <p>
        <strong class="rule-title">No off-roading</strong> The vehicle may have
        a device that collects driving and location data.
      </p>
    </div>
  </div>
</template>
...
// --- NEW COMPUTED: ASSET TYPE CHECK ---
const isMotorcycle = computed(() => {
  return (props.vehicle?.assetType || "").toLowerCase() === "motorcycle";
});
...
const selectedFeaturesList = computed(() => {
  if (
    !props.vehicle ||
    !props.vehicle.features ||
    typeof props.vehicle.features !== "object"
  ) {
    return [];
  }
    ...
    const reviewPromises = snapshot.docs.map(async (reviewDoc) => {
      ...
      if (reviewData.renterId) {
        try {
          ...
          console.error(
            `Failed to fetch user data for renterId: ${reviewData.renterId}`,
            e
          );
        }
      }
...
const totalReviews = computed(
  () => reviews.value.filter((r) => r.categoricalRatings).length
);
