<template>
  <div class="admin-page-container">
    ...
    <div class="content-area">
      ...
      <div v-else-if="paginatedVehicles.length === 0" class="state-container empty">
              ...
              <tr v-for="vehicle in paginatedVehicles" :key="vehicle.id">
                <td>
                  <div class="vehicle-info-cell">
                    <img :src="vehicle.exteriorPhotos[0]" alt="Car" class="vehicle-thumb" />
                    <div class="vehicle-text">
                      <span class="model">{{ vehicle.year }} {{ vehicle.make }} {{ vehicle.model }}</span>
                ...
                <td>
                  <div class="owner-info-cell">
                    <span class="owner-name">{{ vehicle.resolvedOwnerName }}</span>
                    <span class="owner-email">{{ vehicle.resolvedOwnerEmail }}</span>
                  </div>
                </td>
                <td class="price-cell">₱{{ vehicle.rentalPricePerDay.toLocaleString() }}</td>
                <td>
                ...
                <td>
                  <div class="actions-cell">
                    <button @click="viewVehicle(vehicle.id)" class="action-btn view" title="View">
                      <i class="bi bi-eye"></i>
                    </button>
        ...
        <div class="mobile-view grid-layout">
          <div v-for="vehicle in paginatedVehicles" :key="vehicle.id" class="vehicle-card">
            <div class="card-image-wrapper">
              <img :src="vehicle.exteriorPhotos[0]" alt="Car" class="card-image" />
              <span :class="['status-badge card-badge', getStatusClass(vehicle.dynamicStatus)]">
                {{ vehicle.dynamicStatus }}
              </span>
            </div>
            ...
            <div class="card-body">
              ...
              <div class="card-details">
                ...
                <div class="detail-item price">
                  <i class="bi bi-tag"></i>
                  <span>₱{{ vehicle.rentalPricePerDay.toLocaleString() }}/day</span>
                </div>
              </div>
            </div>

            <div class="card-actions">
              <button @click="viewVehicle(vehicle.id)" class="btn-card secondary">Details</button>
...
export default {
    ...
    filteredAndSortedVehicles() {
      ...
      let vehiclesWithStatus = this.allVehicles.map(vehicle => {
        ...
        if (this.allUsers && this.allUsers.length > 0) {
            const owner = this.allUsers.find(user => user.uid === vehicle.ownerId || user.id === vehicle.ownerId);
            if (owner) {
        ...
        return { ...vehicle, dynamicStatus, resolvedOwnerName, resolvedOwnerEmail };
      });
  ...
  methods: {
    ...mapActions(['fetchAllVehicles', 'fetchAllBookings', 'fetchAllUsers', 'deleteVehicle']),
...
.vehicle-info-cell {
  ...
  .vehicle-text {
    ...
    .model { font-weight: 600; color: $text-main; }
    .plate { font-size: 0.8rem; color: $text-light; text-transform: uppercase; }
  }
}
...
.owner-info-cell {
  display: flex;
  flex-direction: column;
  .owner-name { font-weight: 600; color: $text-main; }
  .owner-email { font-size: 0.85rem; color: $text-light; }
}
...
.vehicle-card {
  ...
  .card-body {
    ...
    .card-plate { margin: 0.25rem 0 1rem; font-size: 0.85rem; color: $text-light; text-transform: uppercase; }
    ...
    .card-details {
      ...
      .detail-item {
        ...
        &.price { color: $text-main; font-weight: 600; }
      }
    }
  }
...
.action-btn, .btn-icon {
  ...
  &.view:hover { background: #e0f2fe; color: #0284c7; }
...
.pagination-controls {
  ...
  .pagination-btn {
    ...
    &:not(:disabled):hover { border-color: $primary-color; color: $primary-color; }
  }
  
  .page-info { font-size: 0.9rem; color: $text-light; }
}
